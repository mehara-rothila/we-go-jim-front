...
const Register = () => {
    ...
    const { register: signup, loading, error: authError } = useAuth();
        ...
        <div className={`min-h-screen flex items-center justify-center ${theme === 'dark' ? 'bg-dark-bg text-text-light' : 'bg-light-bg text-gray-800'} transition-colors duration-200`}>
            <div className={`max-w-md w-full space-y-8 p-8 rounded-lg shadow-lg ${theme === 'dark' ? 'bg-dark-card border border-dark-border' : 'bg-light-card border border-light-border'}`}>
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h2 className={`text-3xl font-bold mb-2 ${theme === 'dark' ? 'text-text-light' : 'text-gray-800'}`}>Create Account</h2>
                        <p className={`${theme === 'dark' ? 'text-text-muted' : 'text-gray-500'}`}>Join WE GO JIM to start your fitness journey</p>
                    </div>
                    <ThemeToggle />
                </div>
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
                    <div>
                        <label htmlFor="name" className={`block text-sm font-medium ${theme === 'dark' ? 'text-text-light' : 'text-gray-700'}`}>
                            Full Name
                        </label>
                        <div className="mt-1 relative rounded-md shadow-sm">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <User size={18} className={`${theme === 'dark' ? 'text-text-muted' : 'text-gray-400'}`} />
                            </div>
                            <input
                                type="text"
                    ...
                    <div>
                        ...
                        <p className={`mt-2 text-xs ${theme === 'dark' ? 'text-text-muted' : 'text-gray-500'}`}>
                    ...
                    <div className="flex items-center">
                        ...
                        />
                        <label htmlFor="terms" className={`ml-2 block text-sm ${theme === 'dark' ? 'text-text-muted' : 'text-gray-500'}`}>
                            I agree to the <a href="#" className={`font-medium ${theme === 'dark' ? 'text-text-link' : 'text-blue-600'}`}>Terms and Conditions</a>
                        </label>
                    </div>
                ...
                <div className="text-center pt-2">
                    <Link to="/login" className={`text-sm font-medium ${theme === 'dark' ? 'text-text-link hover:text-blue-400' : 'text-blue-600 hover:text-blue-800'} transition-colors`}>
